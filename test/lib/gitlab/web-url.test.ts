...
describe("buildTagUrl", () => {
  it("プロジェクトのweb URLの配下にタグページのパスを足す", () => {
    expect(
      buildTagUrl(toPlatformUrl("https://gitlab.example.com/group/proj"), toTagName("v1.0.0")),
    ).toBe("https://gitlab.example.com/group/proj/-/tags/v1.0.0")
  })

  it("サブパス設置のインスタンスでもグループ/プロジェクト部分を落とさない", () => {
    expect(
      buildTagUrl(toPlatformUrl("https://example.com/gitlab/group/proj"), toTagName("v1.0.0")),
    ).toBe("https://example.com/gitlab/group/proj/-/tags/v1.0.0")
  })

  it("タグ名の / をエスケープする", () => {
    expect(
      buildTagUrl(toPlatformUrl("https://gitlab.example.com/group/proj"), toTagName("release/1.0")),
...
describe("buildCompareUrl", () => {
  it("2つのタグを ... で繋いだ比較ページのURLを組み立てる", () => {
    expect(
      buildCompareUrl(
        toPlatformUrl("https://gitlab.example.com/group/proj"),
