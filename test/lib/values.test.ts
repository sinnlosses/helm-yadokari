...
describe("setValueAtPath", () => {
  it("トップレベルのキーの値を書き換える", () => {
    ...
    expect(getValueAtPath(result, "tag")).toBe("v2.0.0")
    expect(getValueAtPath(result, "other")).toBe("keep")
  })
  ...
  it("ネストしたdotパスの値を書き換える", () => {
    ...
    expect(getValueAtPath(result, "image.tag")).toBe("v2.0.0")
    expect(getValueAtPath(result, "image.repository")).toBe("my-app")
  })

  it("パスが存在しないとき例外をスローする", () => {
    expect(() => setValueAtPath("image:\n  tag: v1.0.0\n", "image.missing.deep", "x")).toThrow(
      "image.missing.deep",
    )
  })

  it("途中のキーがオブジェクトでないとき例外をスローする", () => {
    expect(() => setValueAtPath("image: v1.0.0\n", "image.tag", "v2.0.0")).toThrow("image.tag")
  })
})
