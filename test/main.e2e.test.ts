...
/** 4つの設定ユニットの `config.yaml` に書かれている `helm.branchRef`（4つとも同じ値） */
...
// `anchor-app`・`tenant2/client2`・`shared-app` の向き先ブランチの現在値は `helm.branchRef`
...
describe("run（config/ の実ファイルを読むe2e）", () => {
  ...
  it("コミットされる values.yaml の中身が、実ファイルの設定どおりに書き換わる", async () => {
    ...
    // Helmの向き先ブランチは config.yaml の helm.branchRef へ書き換わる
