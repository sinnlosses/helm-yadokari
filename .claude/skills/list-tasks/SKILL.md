...
**下のコマンドの出力だけを使う**（`task` 本文の代わりに、要約の材料となる先頭120字だけが出る）。
...
for t in ts:
    body = t["task"]
    m = re.search(r"^\*\*タスク\*\*:\s*(.+)$", body, re.M)
    if m:
        head = m.group(1).strip()
    else:
        head = ""
        for sec in re.split(r"^## ", body, flags=re.M):
            if sec.startswith("背景"):
...
出力は TSV。列は
`id / status / difficulty / dependencies / 着手可否 / passes / 本文の先頭120字`。
...
2. 次の形のテーブル**1つだけ**を出す。行の並びは `todo`（着手可能なものが先）→ `doing` → `done`。
   ...
   - **`内容` 列は自分で要約する。** コマンドが出した先頭120字をそのまま貼らない。
     **30字程度**に圧縮し、「何をするか」が分かる一文にする（例:
     「`ConfigDirPath` を `ConfigRootPath` に改名」）。長い列があるとテーブルが読めなくなる
