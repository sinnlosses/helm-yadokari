...
4. **実行**: 選んだタスクの `difficulty` に応じて委譲する（`docs/workflow.md`
   ...
   - `haiku` / `opus` → Agentツールで該当モデルを指定したサブエージェントに委譲する。
     `tasks.json`の`task`本文だけで作業が完結するよう、対象ファイル・完了条件・
     `pnpm check`を通すことを明記して渡す
   - `sonnet` → メインセッションが自分で実行する（既定モデルが`sonnet`のため）
   - ユーザーへの確認が必要な判断・会話中の文脈に依存する判断は委譲しない。`opus`の
     タスクはこれに当たりやすいので、該当したら委譲せずユーザーに預けて次のタスクへ進む
