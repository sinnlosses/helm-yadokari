...
- **追跡ブランチ由来のタグのうち、追跡ブランチの現在のHEADコミットを指しているものを探す**。
  ...
  - **「HEADを指すタグを直接探す」**方式にしているのは、このツールの目的が「追跡ブランチの
    ...
    無駄なタグを作ってしまう
- `values.yaml` にすでに反映されているタグが、現在の追跡ブランチ由来のタグとしてパースできない
  場合（＝`branchToSync` を切り替えた場合。`TAG_FORMAT` を変更した場合も同様）は、HEADコミットと
  一致する既存タグがあってもそれを再利用せず、必ず新しいタグを作成する。切り替え前後のブランチが
  同じコミットを指していても、追跡先が変わったことを `values.yaml` 上で明示するため更新する。
  まだ何も反映されていない箇所（アンカーが見つからない）はこの判定の対象外とする（書き込み時に
