...
| ### 4.3 複数アプリ・複数chartリポジトリ・複数設定ユニットの処理 | 失敗の波及範囲（トークン別の401を含む）、オールオアナッシング、終了コード、同時実行数 |
| ### 4.4 アプリの登録・設定                                      | **`config/` のスキーマ・制約の正典**。最長の節（YAMLの実例を含む）                    |
| ### 4.5 特定chart・特定設定ユニット限定実行                     | `TARGET_CHART` / `TARGET_UNITS` の絞り込みとエラー条件                                |
| ## 5. 実行環境・非機能要件                                      | 実行形態・配布・認証（グループ単位のトークンの発行と置き場所）・dry-run・実行レポート |
| ## 6. 参照                                                      | 関連ドキュメントへのリンク                                                            |
...
- **アクセストークンに起因する失敗の波及範囲は、そのトークンを使うchartリポジトリまで**。
  chartリポジトリが`registry.yaml`の`accessTokenEnv`で宣言したトークンの401（認証エラー）と、
  宣言した環境変数がそもそも未設定だった場合は、そのchartリポジトリ配下の設定ユニットだけを
  `ERROR`として記録し、他のchartリポジトリの処理は継続する。トークンをグループごとに分けている
  以上、1グループのトークンの期限切れ・付け替え漏れを他グループへ波及させる理由が無いため。
  全chartリポジトリが`accessTokenEnv`を宣言するので、トークン起因の失敗はすべてこの扱いになる
- ただし宣言された環境変数が1つも読めず、アクセストークンが1本も手に入らないときは、実行前の
  組み立てでCLI全体を即時終了する（設定エラーと同じ経路）。chartリポジトリ単位の`ERROR`に
  落とせるのは、他に1本でも読めるトークンがあるときだけのため
- 5xx・ネットワーク障害・リクエストのタイムアウトは、**どのトークンで呼んだかに関わらず**
  従来どおり即時終了する。プラットフォーム側の障害であってトークンの問題ではないため
- 1つの`projectId`が複数のchartリポジトリで別々のトークンに結びつく構成は、4.4節の設定エラー
  として即時終了する（上の「トークンに起因する失敗」には含めない）
...
| ファイル        | スコープ        | 持つもの                                                                                                                                                                       |
| --------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `registry.yaml` | chartリポジトリ | MRの作成先（`chartToUpdate`）、ソースリポジトリのタグ形式の台帳（`appSpecs[].tagFormat`）、操作に使うアクセストークンの宣言（`accessTokenEnv`）、所属グループの宣言（`group`） |
...
group: team-a-group # （必須）このchartリポジトリとソースリポジトリが属するグループのフルパス
chartToUpdate:
...
- `registry.yaml`のトップレベルの`group`は、そのchartリポジトリと`appSpecs[]`のソース
  リポジトリが属するGitLabのグループ（namespace）の**フルパス**を書くフィールドで、**必須**。
  書かれていなければ設定エラーになる。`accessTokenEnv`が「どのトークンを使うか」の宣言なのに
  対し、`group`は「そのトークンがどこまで届いてよいか」の宣言にあたる。必須にする理由も
  `accessTokenEnv`と同じで、書き漏らしたchartリポジトリが黙って照合をすり抜ける形を残さないため
- 書ける値は英数字・アンダースコア・ドット・ハイフンからなるセグメントを`/`でつないだ形
  （例: `team-a-group`・`team-a-group/sub-group`）に限り、空文字・前後や連続の`/`は設定エラーに
  なる。照合がセグメント単位の前方一致で行われるため、末尾に`/`が付いた値は常に外れてしまう
- `group`の照合は`validate-config --remote`（実在チェック）だけが行い、本体の更新処理は参照
  しない。`chartToUpdate.projectId`と`apps[].projectId`のそれぞれについて、GitLabが返す
  プロジェクトの所属（`namespace.full_path`）が`group`そのものか、その**サブグループ配下**で
  あることを確認する。サブグループを許すのは、グループのトークンがサブグループのプロジェクトにも
  届く＝宣言したトークンの被害範囲の内側だから。照合はセグメント単位なので、`team-a-group`は
  `team-a-group-2`に一致しない。外れていた場合は、プロジェクト不在（＝トークンで参照できない）
...
- **トークンの単位はグループ**: 複数チーム（＝複数グループ）が1つの`config/`を共用するため、
  トークンは**グループごとに1本**発行する（`PLATFORM=gitlab`ならそのグループのGroup Access
  Tokenを`read_api` + `write_repository`・Developerロール・短い有効期限で、`PLATFORM=github`なら
...
- **どのトークンを使うかはchartリポジトリ単位に宣言する**: `registry.yaml`の`accessTokenEnv`に
  環境変数名を書き、1回の実行で複数のトークンを使い分ける（4.4節）。宣言は必須なので、CLIが
  読むトークンは`ACCESS_TOKEN_<グループ>`だけになり、どのグループにも属さない広い権限の
  トークンを置く先は残らない
- **Dry-runモード**: 実際のブランチ作成・MR送信を行わず、「何を更新する予定か」だけを
  確認できる `--dry-run` 相当のモードを用意する
- **実行レポート**: バッチ1回分の実行結果を設定ユニット単位の表を持つMarkdown 1枚に書き出し、
  GitLab CI の artifacts として回収する。`DRY_RUN=true` でも書き出す。致命的エラーで
  中断したときは書き出さない
