...
- 判定した最新タグを元に、chartリポジトリの `values.yaml` 内のイメージタグを更新する
  コミットを作成し、`(chartリポジトリ, テナント/クライアント)` 単位でMRを送る
...
| テナント / クライアント       | 同一のchartリポジトリ配下で、アプリの設定をさらに分割管理するための単位。`config/<chartリポジトリ>/<tenantId>/<clientId>/config.yaml` というディレクトリ階層で表現する。MRを作成する単位でもある |
...
- GitLabのタグ一覧から、アプリごとに設定された追跡ブランチ (`branchToSync`) 由来のタグ
  のみを対象に最新版を判定する
- タグの命名規則: デフォルトは `${追跡ブランチ名の "/" を "-" に置換した値}-build-at-${yyyymmdd}-${hhmmss}`
  （例: 追跡ブランチが `release/foo` なら `release-foo-build-at-20260902-123456`）。
  この規則からブランチ名部分を検証し、日時部分をタイムスタンプとしてパースする。
  `TAG_FORMAT`環境変数で`{branch}`/`{date}`/`{time}`プレースホルダを
  ちょうど1回ずつ含むテンプレート文字列を指定すれば、順序・区切り文字を含めてカスタマイズ
  できる。このフォーマットは新規タグ作成・既存タグ判定の両方に使われる
  - **3つのプレースホルダの並び順は任意**で、`{date}-{time}-{branch}` のように
    デフォルトと違う順序でも、区切り文字が異なっていてもよい（例: `v{time}_{branch}__{date}`）。
    3つを各1回ずつ含むという条件だけが必須
- **追跡ブランチ由来のタグのうち、追跡ブランチの現在のHEADコミットを指しているものを探す**。
  見つかればそれを最新タグとして扱い（複数見つかった場合は、いずれも同じコミットを指していて
  デプロイされる中身は同じなので、タグ名から読み取った日時が最も新しいものを選ぶ）、
  1件も見つからなければ、そのアプリの追跡ブランチの最新コミットに対して、このツール自身が
  上記命名規則に従った新しいタグを作成し、それを最新タグとして扱う（`DRY_RUN=true` のときは
  実際のタグ作成をスキップし、作成予定のタグ名だけを使って以降の判定を続ける）
  - **「HEADを指すタグを直接探す」**方式にしているのは、このツールの目的が「追跡ブランチの
    ...
    無駄なタグを作ってしまう
- `branchToSync` を切り替えた場合（`TAG_FORMAT` を変更した場合も同様）も、上記の判定を
  ...
  - タグ名には `{branch}` が必ず含まれるため、そのタグを `values.yaml` に書けば追跡先が
    ...
    必要はない
  - 切り替え前後のブランチが同じコミットを指していても `values.yaml` は更新する。
    切り替え前のタグ名は現在の追跡ブランチ由来としてパースできず、後述の
...
- MRはアプリ単位ではなく、**`(chartリポジトリ, テナント/クライアント)` 単位** で作成する。
  GitLabのMRはプロジェクト単位でしか作れないため、1つのchartリポジトリ配下の
  複数テナント/クライアントは同じプロジェクトに対して**それぞれ独立したMR**になる（1つの
  chartリポジトリに複数のテナント/クライアントがある場合、そのchartリポジトリには複数のMRが
  並ぶことがある）。1つのテナント/クライアント配下で更新対象になった複数アプリの変更は、
  1つのブランチ・1つのMRにまとめる
- ブランチ名は `feature/yadokari/<tenantId>/<clientId>` とする（テナント/クライアント単位で
  ブランチが分かれるため、IDをブランチ名に含める）。`tenantId`/`clientId`にGitLabブランチ名
  として不正な文字が含まれる場合の追加バリデーションは行わず、ブランチ作成APIのエラーを
  4.3節のエラーハンドリング方針（該当分だけERROR）に委ねる
- MRの作成先ブランチ（ベースブランチ）は、そのchartリポジトリの `chart.yaml` に設定する
  `mrTargetBranch` を使う（テナント/クライアントに関わらず、同じchartリポジトリ内では共通）
- MRタイトルは `Auto MR by yadokari: update ${tenantId}/${clientId} ${N} app image tag(s)`
  とし、同一プロジェクトに複数MRが並んでもタイトルだけで見分けられるようにする
- 既にそのテナント/クライアントの固定ブランチにMR（未マージ）がオープンな場合は、その
  テナント/クライアント配下の全アプリの更新処理をスキップする（他のテナント/クライアント・
...
- **既知の制限**: 同じ`values.yaml`（`valuesPath`）を異なるテナント/クライアントのアプリが
  共有している場合、それぞれ別ブランチ・別MRで同じファイルを独立に書き換えることになり、
  マージ順によっては片方の変更が食い違う可能性がある。追加の検証は行わない（テナント/
  クライアントはそもそもアプリ設定を分割管理する単位のため、想定される運用ではほぼ
  起きないケースとして許容する）

### 4.3 複数アプリ・複数chartリポジトリ・複数テナント/クライアントの処理

- 1回のCLI実行（1回のpipeline schedule実行）で、`config/` 配下に登録された全chart
  リポジトリ・全テナント/クライアント・全アプリをまとめて処理する
- **`(chartリポジトリ, テナント/クライアント)`単位間**: あるテナント/クライアントの処理が
  失敗しても、他のテナント/クライアント（同じchartリポジトリ内の別クライアントを含む）の
  処理は継続する
- **`(chartリポジトリ, テナント/クライアント)`単位内（アプリ間）**: 同じテナント/クライアント
  配下のいずれかのアプリの処理（タグ取得等）が失敗した場合は、そのテナント/クライアントの
  更新（MR作成/更新）をオールオアナッシングで見送る。成功した一部のアプリの変更だけを
  反映することはしない。失敗はログ・サマリーに出力し、次回実行時に再度全体を試行する
- 実行終了時に失敗した`(chartリポジトリ, テナント/クライアント)`単位の一覧をサマリー表示し、
  失敗が1件でもあればCLI全体の終了コードを非ゼロにする
- `(chartリポジトリ, テナント/クライアント)`単位の処理は `p-limit` 等で同時実行数を制御する
  （デフォルト3〜5程度、設定ファイルで上書き可能）。GitLab APIのレート制限への配慮のため、
  無制限の並列実行は行わない。chartリポジトリ単位・テナント/クライアント単位の2段階の
  同時実行数制御は導入しない
...
- 通常は `config/` 配下に登録された全chartリポジトリ・全アプリを対象に実行するが、
  `TARGET_CHART` / `TARGET_CLIENTS` を指定することで対象を絞り込んだ実行もできる
  （手動での動作確認・特定チームからの緊急更新依頼など、全件実行が不要な場面向け）
- `TARGET_CHART`: `config/` 直下の特定のchartディレクトリ名を1つ指定し、そのchart
  リポジトリのみを対象にする
- `TARGET_CLIENTS`: `"<tenantId>/<clientId>"` 形式で特定のtenant/clientを指定し、
...
- 指定した`TARGET_CHART`、または`TARGET_CLIENTS`内の各tenant/client組が
  `config/`配下に1件も見つからない場合は、typo等に気づけるようエラーとして即時終了する
  （`TARGET_CLIENTS`に複数指定した場合、1件でも見つからない組があればエラーにする。
