...
- 既定の`ACCESS_TOKEN`（`accessTokenEnv`の宣言が無いchartリポジトリが使うトークン）の401は、
  従来どおりCLI全体を即時終了する。宣言の無いchartリポジトリすべてに共通で効くトークンなので、
  続けても同じ失敗を繰り返すだけになる
...
| ファイル        | スコープ        | 持つもの                                                                                                                                        |
| --------------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `registry.yaml` | chartリポジトリ | MRの作成先（`chartToUpdate`）、ソースリポジトリのタグ形式の台帳（`appSpecs[].tagFormat`）、操作に使うアクセストークンの宣言（`accessTokenEnv`） |
...
```yaml
accessTokenEnv: ACCESS_TOKEN_TEAM_A # （任意）このchartリポジトリの操作に使うトークンの環境変数名
chartToUpdate:
...
- `registry.yaml`のトップレベルの`accessTokenEnv`は、そのchartリポジトリの操作に使うアクセス
  トークンが入っている**環境変数名**（トークンの値そのものではない）を書くフィールドで、**任意**。
  省略したchartリポジトリは既定の`ACCESS_TOKEN`を使う。`chartToUpdate`の中ではなくトップレベルに
...
- `accessTokenEnv`に書ける環境変数名は`^ACCESS_TOKEN_[A-Z0-9_]+$`（例: `ACCESS_TOKEN_TEAM_A`）に
  ...
  既定トークンと同じ`ACCESS_TOKEN`（接尾辞なし）も設定エラーで、既定を使うことは
  「フィールドを省略する」の1通りだけで表す
...
- **1つの`projectId`は1つのトークンにしか結びつけられない。** 同じ`projectId`が
  ...
  宣言した複数のchartリポジトリに登場する場合は設定エラーとして即時終了する。省略（＝既定の
  `ACCESS_TOKEN`）も1つの値として数えるので、片方だけが宣言している場合も設定エラーになる。
...
- **どのトークンを使うかはchartリポジトリ単位に宣言する**: `registry.yaml`の`accessTokenEnv`に
  環境変数名を書き、1回の実行で複数のトークンを使い分ける（4.4節）。宣言の無いchartリポジトリは
  既定の`ACCESS_TOKEN`を使う。したがって`ACCESS_TOKEN`は、宣言の無いchartリポジトリが1つでも
  実行対象に含まれるときだけ必須で、実行対象の全chartリポジトリが宣言していれば未設定でよい
