...
| 節                            | 中身                                                                                        |
| ----------------------------- | ------------------------------------------------------------------------------------------- |
| ## 使うGitLabリソース         | chart/ソースリポジトリのprojectIdと環境変数の対応表、フィクスチャに必要なもの               |
| ## 検証シナリオ               | 2つの設定ユニットで確認する内容（image tag更新＋Helm向き先ブランチ更新／image tag更新のみ） |
| ## 手順                       | reset→setup→検証→dry-run→実行のコマンド列                                                   |
| ## 期待する結果               | 終了コード・summary・MRタイトル・MR本文の形の確認項目                                       |
