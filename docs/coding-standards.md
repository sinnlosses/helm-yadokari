...
| ## コメント                                                  | コメント規約の全体。下の7小節を含む                                             |
| ### 読者で書き分ける                                         | `/** */`は呼ぶ人向け・`//`は実装を読む人向け、という切り分け                    |
| ### 1行目は要約、空けてから詳細                              | 2段落以上のJSDocの形と、要約が1行に収まらないときの合図                         |
| ### 呼び出し元の名前を書かない                               | 呼び出し元の名前が嘘になる理由と、「本体パイプライン」で範囲を書く形            |
| ### コードの数え上げを書かない                               | 行数・件数を書かない理由と、検証手段を添えられるときの例外                      |
| ### 残すかどうかは長さではなく種類で決める                   | **種類別**の判断表（写しは消す）と正典の対応表                                  |
| ### レビューで問う2つ                                        | 昔の話か／呼ぶ人に要るか、の2問と迷ったときの扱い                               |
| ### JSDocのブロックタグは使わない                            | `@param`等を導入しない理由                                                      |
...
`/** */` と `//` は**読者が違う**（[Google TypeScript Style Guide](https://google.github.io/styleguide/tsguide.html)）。

| 記法     | 読者                     | 書くこと                                 |
| -------- | ------------------------ | ---------------------------------------- |
| `/** */` | **その関数・型を呼ぶ人** | 中を開かずに正しく使うために要ることだけ |
| `//`     | **その実装を読む人**     | その行・そのブロックが今の形である理由   |

`/** */` に実装の手順を書かない。手順は本体と、呼んでいる関数の名前が語る。
呼ぶ人が要らない内訳（どの検証が走るか、どの順で呼ぶか）も `/** */` には書かない。

### 1行目は要約、空けてから詳細

`/** */` が要約だけで終わらないなら、**1行目を1文の要約にし、空行を挟んでから詳細**を書く。
`docs/research/comment-conventions.md` で TSDoc の「要約と詳細を分け、詳細側で要約を
繰り返さない」構造を採用したときの形で（ブロックタグだけを採らなかった）、ここに明文化する。
...
- **段落を分けずに要約と詳細を続けて書かない。** 1段落のまま何行も続くと、どこまでが要約か
  読まないと分からない。**この判定は機械化しない**（段落が1つであることと、詳細を持たないことは
  別）
- **全体が2行に収まるなら分けなくてよい。** 読み飛ばすかどうかを決める場面が無く、空行を足すと
  かえって嵩む
- **要約が1行に収まらないのは、要約になっていない合図。** たいていは詳細が混ざっているので、
  2文目以降を下の段落へ送る（行数の上限ではない。上限は置かない方針のまま）
- **詳細側で要約を言い直さない。** 同じ主張を2回書くのがこの構造でいちばん起きやすい

**ファイル全体の説明も同じ形で、`/** _/` で書く**（`//` にしない）。読者は「このファイルを
使う人」なので上の表の`/\*\* _/`側に当たる。置き場所は `import` より前。
...
`/** */` が2段落以上になるときは、**1行目を1文の要約にし、空行を挟んでから詳細**を書く。
`docs/research/comment-conventions.md` で TSDoc の「要約と詳細を分け、詳細側で要約を
繰り返さない」構造を採用したときの形で（ブロックタグだけを採らなかった）、ここに明文化する。
...
- **要約が1行に収まらないのは、要約になっていない合図。** たいていは詳細が混ざっているので、
  2文目以降を下の段落へ送る（行数の上限ではない。上限は置かない方針のまま）
- **詳細側で要約を言い直さない。** 同じ主張を2回書くのがこの構造でいちばん起きやすい
- 段落が1つで収まる `/** */` はそのままでよい。要約だけがある状態なので、空行は要らない

### 呼び出し元の名前を書かない

呼ばれる側は呼ぶ側を知らなくても動く。コメントに呼び出し元のディレクトリ名・ファイル名・
関数名を書くと、**呼び出し元が増えたり移動したりした時点でコメントが嘘になる**。
呼び出し側に触れる必要があるときは「呼び出し側」「呼び出し元」と書く。

| 書きたいこと                           | 書き方                                                   |
| -------------------------------------- | -------------------------------------------------------- |
| 呼び出し側から何がどう見えるか         | 「呼び出し側」「呼び出し元」（名前を挙げない）           |
| 何をここに載せる／載せないかの収録基準 | 置き場所（`steps/`）ではなく範囲（「本体パイプライン」） |
| 呼び出し元が満たしている前提           | **事前条件（契約）として書く**。誰が満たすかは書かない   |
| 誰がこれを使っているかの記録           | 正典（`docs/architecture.md`）へ                         |

「本体パイプライン」は CLAUDE.md 原則3 と同じ語（`index.ts`→`main.ts`→`steps/`）。
`steps/` という置き場所ではなく範囲を指すので、ディレクトリを組み替えても嘘にならない。

**例外は、名前を挙げないと今の前提が確かめられないときだけ。** ただし「呼び出し元がこういう値を
渡す」は事前条件なので、契約として書けば名前は要らない。呼び出し元**ではない**別の場所
（下流で何が壊れるか、`scripts/`、外部ツール）を挙げるのはこの規約の対象外で、
`lib/config/validate.ts` が `resolve-tags` での帰結を挙げているのはそれに当たる。

上位から下位を名指しするのは逆向きなので対象外（`main.ts` が `steps/` の4ステップを
並べてよい）。`docs/` も対象外 — 系全体を上から説明する場所なので、どの層が何を必要と
しているかを書いてよい。
...
| 書きたいこと                   | 書き方                                                        |
| ------------------------------ | ------------------------------------------------------------- |
| 「行数は分ける理由にならない」 | しきい値との関係で書く（「200行を超えても分けなかった」）     |
| 「ここが全部だ」               | **列挙する**。列挙すれば数は読めば分かるので書かない          |
| 外部ライブラリの回数・上限     | 対象外（外部の挙動の記録。変わったら外部側の変更として拾う）  |
| 型が守っている一意性           | 対象外（`brand.ts` の「唯一の生成経路」。破れたら型が落ちる） |

### 残すかどうかは長さではなく種類で決める

行数の目安は置かない（数字を置くと、種類の基準の代わりにその数字が基準として使われるため）。
**真の制約・前提は必要なだけ長くてよい。**
...
| **本体の行・呼び先のJSDocの写し**                          | **消す**（呼ぶ人に要る事実なら呼び元に残す） |
| 昔はこうだった（過去の実装・過去の設計の経緯）             | 長短にかかわらず正典へ移し、コードから消す   |

3行目が一番見つけにくい。**コードが良くなった拍子に、それまで有用だったコメントが写しに
変わる**ためで、書いた時点では正しかったものが後から該当する。`loadConfig()` のJSDocは
本体が段の並びになった時点で13行中10行が写しになっていた（実例は
`docs/research/comment-conventions.md`）。

**「呼ぶ人に要るか」で判断するので、同じ事実が2箇所にあってよい場合がある。**
呼び元の `/** */` と呼び先の `/** */` に同じ制約が書かれていても、呼ぶ人が呼び先を
開かずに済むなら重複ではない。写しかどうかは行の一致ではなく読者で決める。
...
各段落に次を順に問う。1問目で消えなければ2問目へ進む。

1. **この段落はコードの今の挙動を説明しているか、昔の話か。** 昔の話なら正典へ移す。
   この判定は機械化しない（日本語の過去形は「GitLab APIは404ではなく403を返していた」の
   ような外部挙動の記録にも使われ、語での検出は誤検知が多すぎる）
2. **この段落は、本体の行と、呼んでいる関数の名前・JSDocを読んでも分からないことか。**
   分かるなら消す。ただし `/** */` では「**呼ぶ人が**呼び先を開かずに済むか」で判断する

どちらでも決まらない段落は**消さずに残す**。判定手順の穴なので、この節を直す。

### JSDocのブロックタグは使わない

`@param` / `@returns` / `@throws` / `@remarks` を導入しない。TSDocやRust APIガイドラインは
例外条件などを専用セクションに置くことを勧めているが、このリポジトリは全編が日本語の散文で
書かれておりブロックタグの使用は0件、読者もチーム内限定でドキュメント生成器も使っていない。
根拠と検討の詳細は `docs/research/comment-conventions.md`。
