...
| #### `config/`は「スコープ」で2ファイルに分け、変更頻度では分けない           | ファイル構成、タグ形式の置き場所、食い違いを設定エラーにする理由 |
...
| `config/chart-and-apps.ts` | 設定ユニットの `config.yaml` と chartディレクトリの `chart.yaml` の `apps[]` を結合し `ChartAndApps` にする |
| `config/schema.ts`         | 2つの設定ファイル（`chart.yaml` / `config.yaml`）のZodスキーマ                                              |
...
| 特定の技術・外部システム・外部ファイル形式のインターフェースの一部                   | その`lib/`ファイル                               | `GitlabClient`・`ConfigTarget`・`ChartApp`・`EnvConfig`                         |
...
- **Zodスキーマから `z.infer` で導出した型はスキーマと同じファイル**（`ChartApp` は
...
`ChartApp`（`z.infer`由来）・`EnvConfig`（2行目の例）を補って埋めた。
...
`chart.yaml`（chartリポジトリ単位）と `config.yaml`（設定ユニット単位）の2つだけを置く。
...
**タグ形式（`tagFormat`）を `chart.yaml` に置くのは、ソースリポジトリの性質だから。**
...
`projectName` は `chart.yaml` を正典としつつ `config.yaml` にも重複して書く。設定ユニットの
...
**同じ`projectId`のappが複数のchartリポジトリの`chart.yaml`に登録されているとき、
