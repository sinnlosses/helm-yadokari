...
| `lib/config/load-chart-and-apps.ts`（ファイル名）                                     | この論点では変えない（別の論点で改名済み） | ここでの論点はYAMLのファイル名（`chart.yaml`→`registry.yaml`）への追随の要否で、`ChartAndApps` に対応する名前という位置づけは変わらないため据え置いた。のちに「動詞が無く何をするか読めない」という別の論点で現在名へ改名している |
