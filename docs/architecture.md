...
| #### タグ形式はapp単位に`config.yaml`へ置く                                                | 置き場所と、食い違いを設定エラーにする理由   |
...
#### タグ形式はchartリポジトリ単位の`sources.yaml`へ置く

タグ形式は**ソースリポジトリの性質**で、設定ユニットごとに変わる値ではない。だから設定ユニット
単位の `config.yaml` / `anchors.yaml` ではなく、`chart.yaml` と同じ階層の `sources.yaml` に置く。

**以前は `config.yaml` の `apps[]` に置いていた。** そのときの理由は「タグ形式は運用値であって
chart構造ではないので `anchors.yaml` ではない」というもので、`config.yaml` と `anchors.yaml` を
分ける軸を「運用値 vs chart構造」だと見なしていた。**この軸の取り方が誤りだった。**
分割の軸は「変更頻度」（よく変更する値と滅多に変更しない値を混ぜない）であり、その軸で見ると
タグ形式は滅多に変更しない側に属する。`docs/requirements.md` 4.4節が正典。

**設定ユニット単位ではなくchartリポジトリ単位にしたのは、重複を消すため。** 同じソース
リポジトリを複数の設定ユニットが追跡する構成では、設定ユニット側に置くと同じ `tagFormat` を
その数だけ書くことになる（移行前の `config/` では、ソースリポジトリ2つに対して5箇所に同じ値が
書かれていた）。`config/` 直下にグローバルな台帳を1つ置く案もあったが、chartリポジトリごとに
独立させるほうがMRの影響範囲がそのchart配下に閉じるため、chartリポジトリ単位を採った。
その代わり、同じソースリポジトリを複数のchartリポジトリが参照する構成では重複が残るため、
後述の食い違い検証は残している。

`projectName` は `sources.yaml` を正典としつつ `config.yaml` / `anchors.yaml` にも重複して
書く。各ファイル単体で「どのappの設定か」が読めることを優先し、食い違いは設定エラーで防ぐ。
...
**同じ`projectId`のappが複数のchartリポジトリの`sources.yaml`に登録されているとき、
