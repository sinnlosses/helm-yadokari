...
**このファイルは通読しない**（21KB超）。下の索引で節を1つ特定し、その節だけを
`sed -n '/^## 埋めない穴（5件）/,/^#\{2,4\} /p' docs/history/test-inventory.md` の形で読む。

| 節                                             | 中身                                               |
| ---------------------------------------------- | -------------------------------------------------- |
| ## 削除候補（9件）                             | 冗長と判定したテストと、その理由・skip確認の結果   |
| ## 重複の集約候補（削除ではない・2件）         | `test/helpers.ts` に寄せる `vi.mock` の準備        |
| ## 要調査（1件）                               | 判断を保留したもの                                 |
| ## 消さないと決めたもの（3件）                 | 回帰テスト・仕様由来で残したものと、その根拠       |
| ## 追加候補（4件）                             | 足すと決めた穴                                     |
| ## 埋めない穴（5件）                           | カバレッジを埋めないと決めた分岐と理由             |
| ## 実施結果                                    | 実際に消した/足した結果。下の小節を含む            |
| ### 追加テストで見つかった食い違い（修正済み） | 追加テストが実際に見つけたバグ                     |
| ## 要件シナリオとの突き合わせ                  | `docs/requirements.md` 4.1〜4.5 と既存テストの対応 |
