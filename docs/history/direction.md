...
タスク化にあたってユーザーへ確認し、`## 決まっていること（蒸し返さない）` に焼き込んだ4点:
`group:` の下にネストして `groupId` + `groupName` を持つ / 名前側のフィールド名は `groupName` /
`groupName` が実物とズレていたら `--remote` で問題として報告する /
`config/` の2件に書く `groupId` は `133749373`。
...
生成したタスク: T-263（`registry.yaml` に `group` を必須で足し、`validate-config --remote` で
プロジェクトの所属を検証する）。タスクにしなかった項目: なし。

ユーザーの生の言い回し（会話から。指示メモはこの会話を受けてエージェントが書き起こした）:

> アクセストークンにグループアクセストークンを設定するのなら、設定値が特定のグループに
> 所属していることを検査しないといけないよね。多分その機能はないと思うんだけど、どうかな

> アクセストークンの、ではなくconfig.yamlに記載したプロジェクトが、特定のグループに所属して
> いて、かつグループアクセストークンでアクセスできること、かな?

エージェントが (a) エラー文言を「不在/権限不足」に分ける / (b) `group` フィールドを足して所属を
照合する の2案を出したのに対し「bだと思うな」。登録時に「`group` は必須にするか任意にするか」を
聞き、「必須にする」を選択（T-263 の `## 決まっていること` に焼き込み済み）。

当時の指示メモ:

## config のプロジェクトがグループに所属しているかを検査する

`registry.yaml` にグループを宣言するフィールド（例: `group: my-group/sub`）を追加し、
`validate-config --remote` で `chartToUpdate.projectId` と `apps[].projectId` が
そのグループ配下にあるかを検証する。

- 照合は `Projects.show` の戻り値の `namespace.full_path` を宣言したグループと前方一致で見る
  （`projectExists()` は今この戻り値を捨てて `true` を返しているだけなので、API呼び出しは増えない）
- 狙いは「トークンが想定より広い」ケースを捕まえること。親グループのトークンやボットのPATに
  差し替わると、アクセス可否の検査（今の「見つかりません」）は通るのに被害範囲の分離だけが
  静かに壊れる。gitlab.com Free では Group Access Token を発行できずボットPATで代替する運用
  なので、この穴は常時開いている
- 所属していないときのメッセージは「見つかりません」と別の文言にする（不在と所属違いは
  直す手が違う）
- `PLATFORM=github` は `validate-config --remote` 自体が未対応なので今回の対象外
- 新フィールドの必須/任意、`config.example/` と README「設定」章・`docs/requirements.md` 4.4節の
  スキーマ追随も範囲に含める

## 2026-09-16（14回目）

生成したタスク: T-259（`docs/coding-standards.md` の重複ブロック解消）、
T-260（JSDocのブロックタグ禁止規約の廃止。指示3に対応）、
T-261（関数の並び順を oxlint で縛れるか調査。指示2に対応）、
T-262（対応記録・履歴の削除。指示1に対応）。タスクにしなかった項目: なし。

T-259 は指示メモに無い。指示1・3のために `docs/coding-standards.md` の現物を読んだところ、
コミット 79b7356 がこのファイルに「削除0行・追加157行」の差分を入れており、53〜208行が
本文と索引表の重複コピーになっていた。`1〜52行 + 210〜667行` が 79b7356 の親 66d86d3 と
完全一致することを diff で確認したため、**79b7356 が意図した規約の更新は重複側にしか
入っておらず正規の本文に反映されていない**と判定し、指示1〜3を載せる前提として先に直す
タスクにした。

登録時にユーザーへ聞いて決めたこと（各タスクの `## 決まっていること` に焼き込み済み）:

| 論点                                    | 決めたこと                                         |
| --------------------------------------- | -------------------------------------------------- |
| 指示1の「ルールのみ」の範囲             | 履歴だけ落とし、ルールの理由・例外は残す           |
| 落とした履歴の行き先                    | どこにも退避せず削除（git 履歴に残るため）         |
| 指示2で oxlint に既製ルールが無かったら | 見送る（`scripts/lint/` に自作チェックは足さない） |

指示メモ（当時の記述のまま）:

- coding-standards に対応したことや履歴は不要で、ルールのみ記載してほしい
- 関数の並び順をlintなどで防げるか調べてできるなら導入、導入できた場合はcoding-standardsの記述は不要だと思うので削除してほしい
- coding-standardsのJSDocのブロックタグは使わない規約をなくし、必要なら記載する方針としたい
...
## 2026-09-15 パス5用のグループB・プロジェクト・トークンを作るスクリプト

グループB（**トップレベル**）とプロジェクト2つ、フィクスチャ、Group Access Token の発行を
GitLab API で一気にやる一回きりのスクリプトを `scripts/smoke/` に足す。認証は **`api` スコープの
個人 PAT を `.env` の `ACCESS_TOKEN` に置く**（ユーザー決定）。既定は dry-run、`--apply` で作成。
既存プロジェクトには一切書かない（同名グループがあれば中止）。`config/README.md`・
`docs/smoke-test.md` の「プロジェクト作成はスクリプトに足さない」は「作成専用の別スクリプトに
隔離する」に更新する。

## 2026-09-15（8回目）

生成したタスク: T-248（`docs/smoke-test.md` にパス5「複数グループ（宣言トークン）」を書く）。タスクにしなかった項目: なし。`config/` への2グループ目の実ファイル追加はプロジェクトIDが決まってから（手順中はプレースホルダ）。

## 2026-09-15 本物のグループ2つで宣言トークンを実機検証する手順をまとめる

「片方の chart の宣言トークンだけ 401 で、もう片方は続行する」を実機で確かめたい。今のスモーク
フィクスチャは1グループ（`sinnlosses-group`）で、2 chart が同じソースリポジトリを共有しているため
別トークンに分けられない。**2つ目のグループ**（chart リポジトリ1つ＋ソースリポジトリ）と
グループごとの Group Access Token を用意し、`config/` に2グループ目の chart ディレクトリを足して、
`docs/smoke-test.md` に「パス5: 複数グループ（宣言トークン）」として手順・期待する結果・後片付けを
書く。
...
`lib/github/` の実装3件・配線1件・ドキュメント追随1件）。

指示はチャットで出されたため `develop/direction.md` を経由していない。**方針決め（T-220）を
前段に切り出し、残り7件を `dependencies` で後ろに置いた**。T-220 は5つの論点
（`steps/` の型・`lib/` の配置・`batch-cache.ts`・中立化後の型名・環境変数の形）をまとめて
決めるタスクで、後続が名前や形を自分で決め直さずに済むようにしてある。

着手前の確認でユーザーが決めたこと:

- **GitLab と GitHub の両方に対応するが、1回の実行で混在はさせない**（forge の選択は
  環境変数1つで全体に効く。chartリポジトリ単位の指定にはしない）
- `config` の `projectId` は**スキーマで数値と文字列の両方を受ける**（既存の `config/` を
  書き換えない。アシスタントの判断で決め、報告した）
- 検証範囲は **`pnpm check` まで**。GitHub実機のスモークテストは別途（GitLab側も未実施のため
  揃える。アシスタントの判断で決め、報告した）

- やろうかなと思うからタスク化してくれる?（GitHub対応。T-219 の計測結果を受けて）
- 両方に対応するだけで、併用はサポートしないよ。

## 2026-09-12（4回目）

生成したタスク: **T-219**（`ProjectId` の中立化の計測、`opus`、`loopable: "N"`、依存なし）。

指示は4項目だが、1つの計測に対する範囲指定（何を測るか・何を測らないか）なので**1タスクに
統合した**。方針決め（どの表現を候補にするか）は計測の入力であって後段ではないため、
前段タスクとして切り出さずタスク本文の「解くべき論点」に置いた。タスクにしなかった項目は無し。

着手前の下調べで、指示の前提を崩す事実は見つからなかった（`src/types/brand.ts` の
`ProjectId` は `number` のブランド型のまま、`src/lib/config/schema.ts` は `z.number().int()` の
ままで、いずれも 2026-09-12 の調査時点から変わっていない）。
