...
- **英語識別子**: `tagNaming` / `TagNaming`（`AppConfig`のフィールド。テンプレート文字列自体は`TagFormat`）
- **定義**: アプリ（ソースリポジトリ）ごとに`config.yaml`の`apps[]`で指定する、タグ名の読み方と
  作り方の規則。`template`モード（`{branch}`と`{date}`はちょうど1回必須、`{time}`だけ0回か1回の
  テンプレート文字列。既定は`{branch}-build-at-{date}-{time}`）と`semver`モードの2つがある。
  `validateTagFormat()`/`parseTag()`/`buildNewTag()`が扱う。仕様は`docs/requirements.md` 4.1節が正典。
- **表記ゆれ**: 当初は固定フォーマット、次に`TAG_FORMAT`環境変数で実行全体に1つ、という段階を
  経てapp単位の設定に移った。環境変数`TAG_FORMAT`は廃止されている。

### 順序キー

- **定義**: HEADを指すタグが複数あるときに1件を選ぶための比較値。`template`モードでは
  打刻日時、`semver`モードではsemverの版。順序キーが同値のとき（`{time}`を含まないテンプレートで
  同じ日付のタグが複数あるとき、semverで同値の版が複数あるとき）はタグ名の降順で決める。
  **デプロイされる中身はどれも同じで、返す値を一意にするためだけの規則**。

### 打刻日時 / ビルド日時

- **英語識別子**: なし（`ParsedTag`が持つのは順序キー`orderKey`で、打刻日時はその中身。
  `builtAt`という名前は`parseTag()`内のローカル変数としてのみ残っている）
- **定義**: タグ名に含まれる`yyyymmdd`/`hhmmss`部分が表す日時。JST（UTC+9固定）で組み立て・
  解釈する。`template`モードでのみ読み取れる（`semver`モードの順序キーはsemverの版）。`{time}`を
  含まないテンプレートでは日付までの精度になる。
...
- **定義**: 追跡ブランチ由来のタグのうち、追跡ブランチの現在のHEADコミットを指しているもの
  ...
  上述の「順序キー」が最大のものを選ぶ）。「追跡ブランチ由来」は`template`モードでは
  タグ名がパースできること、`semver`モードではHEADコミットを指していること自体で判定する。
  1件も見つからない場合は、新規作成されたタグがこれに当たる（後述の「タグ自動作成」）。
  自動作成できない命名規則の場合は最新タグが決まらず、そのアプリはその実行では更新されない。「タグ名が最も新しいものを選んでからHEADと比較する」
...
- **前提条件**: **タグ命名規則が秒単位で一意な名前を生成できるときだけ働く**。すなわち
  `template`モードで`{time}`を含む場合のみ。`semver`モードや、`{time}`を含まない
  テンプレートでは自動作成しない（生成名が一意にならず
  既存タグと衝突するため）。作成しない場合、そのアプリは警告ログを残して更新対象から外れる。
