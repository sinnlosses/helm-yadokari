 ...
 * MR本文のイメージタグ表の1行分。
 *
 * 1アプリが複数箇所を書き換える場合は同じ`plan`のエントリが箇所の数だけ並ぶ。
 * `webUrl`は`update`のリンク（タグ・比較）に使う解決済みの値。
 ...
 * 1つのMRに載せる項目。
 *
 * `collectMrEntries()`が組み立て、`buildMrContent()`がタイトルと本文にする。
 * タイトルの件数と本文のテーブルの行を同じ配列から数えるための形。
