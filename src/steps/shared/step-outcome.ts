 ...
 * アプリ1件分の処理結果。
 *
 * 失敗を例外ではなく値として持つため、1回だけ実行した処理の結果を成功・
 * 失敗のどちらでも複数の設定ユニットへ配れる（例外は最初の1つにしか届かない）。設定ユニット単位の
 ...
 *
 * 捕捉した例外はこのツールのエラー方針に従って処理され、
 * fatalなら`FatalError`として投げ直され（実行全体が止まる）、
 * それ以外は`ERROR`のsettled outcomeになる。
 *
 * 各stepでは`mapWithConcurrency()`の直下で呼び、「並列に実行する」ことと「1件ずつ失敗を封じ込める」
 * ことがstepの入口に並んで見えるようにしている。
 *
 ...
 *
 * オールオアナッシングで設定ユニット全体がERRORになるため、
