 ...
 * 2ブランチ構成である、という前提のため`helm`自体を必須にする。書き込む値（`branchName`）と
...
const HelmSchema = z.object(
  {
    branchName: z
      .string({
        error: "helm.branchName は必須です（helm.locations とセットで指定してください）",
      })
      .min(1, "helm.branchName は空にできません")
      .transform(toBranchName),
    locations: z
      .array(AnchorLocationSchema, {
        error: "helm.locations は必須です（helm.branchName とセットで指定してください）",
      })
  ...
  },
  {
    error:
      ...
      "2ブランチ構成のため、config.yaml に helm.branchName（向き先ブランチ名）と " +
