...
/**
 * プロジェクトのweb URL配下のページURLを組み立てる。
 *
 * `webUrl`はオリジンではなく**プロジェクトのパスまで含んだURL**（`https://host/group/proj`、
 * サブパス設置なら`https://host/gitlab/group/proj`）なので、
 * `new URL(path,webUrl)`ではなく連結で組み立てる（前者はベースのパスを捨ててしまう）。
 * タグ名のエスケープもここに閉じ込め、呼び出し側が`encodeURIComponent`を書かなくて済むようにする。
 */
