 ...
 *
 * 値そのものではなく実行中の Promise をキャッシュするため、
 * 同じキーの fetch が同時に走っても呼び出しは1回で済む。失敗した Promise はキャッシュから取り除き、
 * 次の呼び出しで再試行できるようにする。
 *
...
/**
 * 読み取り1つをキャッシュ付きの関数にする。
 *
 * キーは引数から機械的に組み立てるので、呼び出し側がテンプレート文字列を手書きしなくてよい。
 * 読み取りごとに`Map`を分けるため、別の読み取りとのキー衝突は起こらない。
 * 同じキーの問い合わせが同時に来ることがあるため、値ではなく実行中のPromiseを共有する
 ...
 * 引数からキャッシュキーを組み立てる。
 *
 * 区切りにヌル文字を使うのは、通常の文字列引数（IDやパス、名前など）
