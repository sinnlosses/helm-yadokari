 ...
 */
export const MAX_CONFIG_UNIT_DEPTH = 2
...
/**
 * 「どの設定ユニットで起きたか」の表示用文字列（`<chartDirName>/<unitPath>`）を組み立てる。
 *
 * ログ・エラーメッセージ・検証結果の報告でのみ使う表示専用の値のため、
...
export function buildConfigUnitLocation(
  ...
  return `${chartDirName}${CONFIG_UNIT_PATH_SEPARATOR}${unitPath}`
}
