...
flowchart TD
    ...
    D -->|なし| F[[配下の全アプリを処理]]
    F --> G1{helmの設定タグは追跡ブランチ由来?}
    G1 -->|No（追跡ブランチを切り替えた）| H[🏷 追跡ブランチの最新コミットに新しいタグを作成]
    G1 -->|Yes| G2{追跡ブランチのHEADを指すタグが存在する?}
    G2 -->|No| H
    G2 -->|Yes| I{helmの設定タグは最新タグと同じコミットを指す?}
    I -->|Yes（タグ名が違っても中身は同じ）| E2["⏭ SKIPPED（no_diff）<br/>全アプリが更新不要のとき"]
    I -->|No| J[✅ 差分のあるアプリだけhelmを更新しMR作成]
    H --> J
```
