...
- **タグ自動作成** — 追跡ブランチ由来のタグがまだ無い場合、そのブランチの最新コミットに命名規則通りの新しいタグを作成してから更新する
...
flowchart TD
    ...
    G -->|見つからない| H[🏷 追跡ブランチの最新コミットに新しいタグを作成]
    H --> I
    G -->|見つかる| I{values.yaml のタグと最新タグは一致?}
    I -->|全アプリ一致| E
...
| 追跡ブランチ由来のタグが見つからない                   | エラーにせず、そのブランチの最新コミットに新しいタグを作成して続行 |
