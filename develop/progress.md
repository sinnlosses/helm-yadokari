...
5タスクとして登録した。前回までの流れは下の「完了したこと」を参照）
...
- **T-123（e2eテストの方針決め、`opus`）** → **T-124（その実装、`sonnet`、T-123依存）**。
  T-123 が「足さない」と結論したら T-124 も同時に閉じる。
- **T-125（パラメータ化候補の洗い出し、`sonnet`）** は依存なし。実装はせず
  `develop/parameterization-candidates.md` を出すところまで。
- **T-127（`apply-updates` のサブステップ粒度と `gitlab.ts` の薄いラッパー化、`opus`）** は依存なし。
  既存の設計判断を覆すかの判断を含むので、**覆さない結論もありうる**。
- **T-126（`config/` の運用方針、`opus`）だけは `/loop /next-task` に載せない**
