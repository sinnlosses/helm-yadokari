...
キャッシュの取りこぼしを棚卸しして T-111〜T-115 を登録した）
...
- **キャッシュの取りこぼしの解消（T-111〜T-115、会話由来の棚卸し）**。各ステップのGitLab
  問い合わせを洗い出したところ、バッチ全体で使い回せるのに毎回問い合わせている箇所が4件
  見つかった。前段として**キャッシュ機構そのものの設計（T-111、`opus`）**を置き、その上に
  個別の適用（T-112〜T-115、`sonnet`）を積む依存関係にしてある。**T-111 は方針決めを含むので
  `/loop` の自動進行に載せず、ユーザーがいるセッションで扱う**。
