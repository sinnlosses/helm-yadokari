...
**未着手のタスクは4件**（コメント規約の見直し。方針1件＋適用3件）:

- **T-187**（`opus`、依存なし、**委譲しない**）: プロフェッショナルなコメント規約を一次情報で
  調べ、`docs/coding-standards.md`「コメント」節を書き換える。**正典の書き換えなので
  ユーザー承認が要る**。既存節の「制約・前提は必要なだけ長くてよい」と今回の要望が衝突しうる
  のが最大の論点
- **T-188**（`sonnet`、T-187依存）: `src/lib/` に適用（コメント 349行、最大）
- **T-189**（`sonnet`、T-187依存）: `src/steps/` に適用（257行）
- **T-190**（`sonnet`、T-187依存）: `src/domain/`・`src/types/`・`src/utils/`・`scripts/` に
  適用（308行）。`test/` は実測3%なので対象外

T-172〜T-186 はすべて `done`。T-176・T-177 は着手しない判断で閉じたもので、理由は下の
「未解決」にある。
