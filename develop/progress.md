...
**未着手は T-172・T-173 の2件だけ**（下の「次にやること」）。完了タスクは
...
**残るは T-172 と T-173 の2件だけ**（棚卸しの13件と T-171 は全件完了）。T-172 は承認が要るので
`/loop /next-task` に載せない。T-173 は T-172 の結論待ちで、「改名しない」なら丸ごと不要になる。
