...
**未着手は7件**（T-172・T-173 と、2026-09-10 の指示から起こした T-175〜T-179。
下の「次にやること」）。完了タスクは
...
**未着手は7件**。うち **T-172・T-176・T-178 は方針決め・承認を含むので `/loop /next-task` に
...
- **T-175（アクセストークンがログに出うる経路の確認とマスクの隙間塞ぎ、`sonnet`、依存なし）**。
  `src/utils/logger.ts` の `redact()` はトップレベルのキー名の**完全一致**でしか伏せないため、
  `EnvConfig` が使っている **`accessToken` というキー名は現状ヒットしない**。ただし今の
  呼び出し元13箇所は誰もトークンを渡していないので、**「今は漏れない」と確かめて閉じる結論も可**。`/loop` 可。
