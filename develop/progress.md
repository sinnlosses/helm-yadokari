...
- **T-109・T-110 の2件を登録した（どちらも `todo` / sonnet / 依存なし）**。ユーザーからの追加指示。
  - **T-109**: 実機スモークテストを実行できる状態が揃っているかの確認。認証情報の有無・
    3プロジェクトの実在・手順に出てくるコマンドの実在・`config-test/` のフィクスチャ・
    「期待する結果」の記述のズレの5点。**このタスクでは実行しない**（書き込みを伴う手順は
    人間の承認が要るため、確認と報告まで）
  - **T-110**: コミットメッセージにタスクIDを振る運用にできるかの確認と、できる場合の
    ルール整備。論点は `docs/coding-standards.md`「タスク番号を書かない」との整合。
    アーカイブ後もIDは `docs/history/` に残るのでコード側の懸念はそのままは当てはまらない、
    というのが出発点。過去コミットは遡って書き換えない
- T-102〜T-108（品質の棚卸し）は全件完了済み。
