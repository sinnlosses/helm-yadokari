...
**未着手は3件**（T-172・T-173・T-179。下の「次にやること」）。完了タスクは
...
**未着手は3件**。うち **T-172 は承認が要るので `/loop /next-task` に載せない**（ユーザーがいる
セッションで扱う）。T-173 は T-172 の結論待ちで、「改名しない」なら丸ごと不要になる。
**T-179 は依存が外れたので `/loop` で進められる**。T-176・T-177 は着手しない判断で閉じた（下の「未解決」）。
