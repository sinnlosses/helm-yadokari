...
**`todo` は5件**（T-214〜T-218。T-212 の提案の反映）。ユーザーが採否を決め、
**L-5 はMIT・D-4は「npm配布しないので正典を実装に合わせる」**で確定した（2026-09-12）。
残り15件は全件採用。README.md を触る3件は衝突を避けるため
`T-214（ズレの訂正）→ T-215（重複の整理）→ T-216（不足の追加）→ T-217（MIT）`の直列で、
T-218（`docs/requirements.md` 5章）だけ独立して進められる。
