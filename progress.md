...
最終更新: 2026-09-06（200行超ファイルの整理をT-044〜T-047として登録、`tasks.json`に`difficulty`を追加）
...
- **3回目のリファクタリング監査（200行超のファイル、2026-09-06）**: ユーザー依頼「200行を
  超えるファイルを整理してもらいたい。責務を明確に、可読性とどこになにがあるか把握しやすい
  ことを重視して」を受けて `src/`・`test/` の200行超9ファイルを調査し、T-044〜T-047 を登録
  （コード変更なし。調査時点で `pnpm check` は21ファイル304テスト全passのクリーンな状態）
  - T-044 `lib/gitlab/gitlab.ts` 364行 → APIラッパーとMR文面の組み立て（外部I/Oを持たない
    Markdown生成）の2ファイルへ。**T-041が既に想定していた分割先の名前
    `test/lib/gitlab/mr-content.test.ts` と1:1で対応する**ため、T-041より前に置いた
  - T-045 `lib/config.ts` 380行 → 既存の `lib/gitlab/` と同じ形の `lib/config/` 4ファイルへ
  - T-046 `lib/verify-config.ts` 227行 → `verifyApp()` の切り出しとキャッシュ層の分離。
    T-042（同ファイルの並列化）の前に済ませると並列化の単位が既に関数になっていて手戻りが
    少ないため、T-042の依存に T-046 を追加した
  - T-047 `types.ts` 267行 → ブランド型を `types/brand.ts` へ（再エクスポート1行で25ファイルの
    importは不変。`as` キャストが1ファイルに閉じる副次効果を狙う）
  - T-041 に `test/steps/build-plans.test.ts`（609行。元の対象から漏れていた）を追加し、
    依存に T-044・T-045 を足した
  - **`src/steps/build-plans.ts`（272行）は分割しない判断**: docs/architecture.md に
    「以前 `sub-steps/build-plans/app-update-plan.ts` に切り出したが、サブステップが
    サブステップを呼ぶ構造になるため非公開関数に戻した」という記録があり、行数を理由に
    再挑戦すると同じ地雷を踏む。関数5つそれぞれにJSDocがあり階層も3段に整理済みなので
    タスク化しない。`test/lib/gitlab/tag.test.ts`（218行）も、`describe`4つが対象関数と
    1:1で対応していて探しやすいため現状維持

- **`tasks.json` に `difficulty` フィールドを追加（2026-09-06、ユーザー依頼）**:
  `haiku`/`sonnet`/`opus` の3段階で、タスクの難易度に応じて実行モデルを切り替える運用を
  想定したもの。**「作業量」ではなく「必要な判断の質」で決める**基準と運用ルール
  （迷ったら1段上／着手して想定より判断が必要なら上げてから再開）を docs/workflow.md に定義し、
  CLAUDE.mdの作業手順にも追記。未完了の9タスクに付与し、`done` 38件には遡って付けていない
  （実行前の見積もりに使う値で、完了後は `evidence` が実績を語るため）。内訳は
  haiku 1件（T-047）／sonnet 6件（T-039・T-040・T-041・T-044〜T-046）／
  opus 2件（T-038・T-042）

## 次にやること

- **T-038（優先・opus）**: CIの `validate-config-remote` が架空の設定例で必ず失敗する問題。
  「`config/` に何を置く運用にするか」の方針決めが先
- **200行超ファイルの整理**: T-044（gitlab分割）→ T-045（config分割）→ T-041（テスト分割）
  の順が最も手戻りが少ない。T-046（verify-config）→ T-042（並列化）も同様の順序依存がある。
  T-047（brand抽出）は依存なしでいつでも着手できる
- T-039（ドキュメント同期）、T-040（未使用コード）
